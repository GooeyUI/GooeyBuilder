...
function onPreviewDragMove(e) {
  ...
  newX = Math.max(
    0,
    Math.min(newX, workspaceRect.width - state.previewWindow.offsetWidth),
  );
  newY = Math.max(
    0,
    Math.min(newY, workspaceRect.height - state.previewWindow.offsetHeight),
  );
...
function updatePropertiesPanel() {
  ...
  document.getElementById("prop-type").value =
    state.selectedWidget.dataset.type;
  ...
  document.getElementById("prop-width").value =
    parseInt(state.selectedWidget.style.width) || rect.width;
  document.getElementById("prop-height").value =
    parseInt(state.selectedWidget.style.height) || rect.height;
  ...
  if (
    ...
    document.getElementById("prop-text").value =
      state.selectedWidget.textContent || "";
  ...
  if (state.widgetCallbacks[widgetId]) {
    document.getElementById("prop-callback").value =
...
function applyWidgetProperties() {
  ...
  if (state.selectedWidget.dataset.type === "Image") {
    state.selectedWidget.dataset.relativePath = document.getElementById(
      "image-relative-path",
    ).value;
    state.selectedWidget.textContent = state.selectedWidget.dataset.relativePath
      .split("/")
      .pop();
  }

  if (state.selectedWidget.dataset.type === "DropSurface") {
    state.selectedWidget.dataset.dropsurfaceMessage = document.getElementById(
      "dropsurface-message",
    ).value;
    state.selectedWidget.textContent =
...
async function generateC() {
  ...
  function processWidgetC(widget, indent) {
    ...
    switch (type) {
      ...
      case "HorizontalLayout":
        let layoutType =
          type === "VerticalLayout" ? "LAYOUT_VERTICAL" : "LAYOUT_HORIZONTAL";
  ...
  document.getElementById("status-text").textContent =
    "C code exported and downloaded";
  setTimeout(() => {
    document.getElementById("status-text").textContent = "Ready";
  }, 2000);
}
document
  ...
  .addEventListener("click", async () => await generateC());
      ...
      try {
        ...
        if (windowElement) {
          state.previewTitleBar.querySelector(
            ".preview-title-text",
          ).textContent = windowElement.getAttribute("title") || "My Window";
          ...
          function loadWidget(widgetElement, parent = null) {
            ...
            if (type === "Slider") {
              ...
              widget.dataset.showHints =
                widgetElement.getAttribute("showHints") || "false";
            } else if (type === "Image") {
              ...
              widget.dataset.dropsurfaceMessage =
                widgetElement.getAttribute("dropsurfaceMessage") ||
                "Drop files here..";
